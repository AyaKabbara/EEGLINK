% pop_importbids_AK() - Import BIDS format folder structure into an EEGLAB
%                    study.
% Authors: Arnaud Delorme, SCCN, INC, UCSD, January, 2019
% Modified for EEGNET3 usage by Aya Kabbata, June 2021

function [MyAllEEGs,Mynames] = pop_importbids_AK(bidsFolder, varargin)
...
for iSubject = 2:size(bids.participants,1)
    timeLaps=1/size(bids.participants,1);
    waitbar(timeLaps*size(bids.participants,1),f,['Loading subject number ' num2str(iSubject)]);
            ...
            for iFile = 1:length(eegFileRawAll)
                ...
                if ~strcmpi(fileExt, '.set') || strcmpi(opt.bidsevent, 'on') || strcmpi(opt.bidschanloc, 'on') || ~strcmpi(opt.outputdir, bidsFolder)
                    ...
                    if strcmpi(opt.metadata, 'off')
                        ...
                        MyAllEEGs{total}=EEG;
                        Mynames{total}=bids.participants{iSubject,1}
                        total=total+1;
