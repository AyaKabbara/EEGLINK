function varargout = preprocess_many_subjects(varargin)
...
% Last Modified by GUIDE v2.5 23-Aug-2022 16:18:25
...
set(handles.figure1, 'Name', 'Preprocess subjects');
...
for e1=1:length(sel_epochs)
    ...
    type=splits{3};
    epoch=str2double(type(6:end));
    IndSegments(e1)=epoch;
    subjIndex_in(e1)=subjIndex;
%     EEG{i}=allepochs{subjIndex_in,IndSegments(i)};
end

closereq;
[params, Visparams, CVG]=tryDefaults();
settingsGUI(params, Visparams, CVG,IndSegments,subjectsParent,subjIndex_in);        
