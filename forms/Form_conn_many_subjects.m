...
set(handles.figure1, 'Name', 'Connectivity for multiple subjects');
...
for e1=1:length(sel_epochs)
    ...
    if(strcmp(type(1),'P'))
           ...
           EEG_in{e1}=allepochs_preprocessed_src{subjIndex(e1),ep};
           isPreprocessed(e1)=1;
           IndSegments(e1)=ep;
           srate(e1)=alleegs{subjIndex(e1)}.srate;


    else
            if(strcmp(type(1),'e'))
            ep=str2double(type(6:end));
            EEG_in{e1}=allepochs_src{subjIndex(e1),ep};
           isPreprocessed(e1)=0;
           IndSegments(e1)=ep;
           srate(e1)=alleegs{subjIndex(e1)}.srate;
...
Form_connectivity(EEG_in,srate,subjectsParent,1);  
closereq;
